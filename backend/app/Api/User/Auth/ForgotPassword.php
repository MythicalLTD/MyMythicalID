...
$router->add('/api/user/auth/forgot', function (): void {
    ...
    if ($appInstance->getConfig()->getSetting(ConfigInterface::TURNSTILE_ENABLED, 'false') == 'true') {
        ...
        if (!Turnstile::validate($cfTurnstileResponse, MyMythicalID\CloudFlare\CloudFlareRealIP::getRealIP(), $config->getSetting(ConfigInterface::TURNSTILE_KEY_PRIV, 'XXXX'))) {
